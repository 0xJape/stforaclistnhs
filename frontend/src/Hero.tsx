...
export default function Hero({rows,onEnter,leaving}:{rows:HeroRow[];onEnter:()=>void;leaving:boolean}){
 const container=useRef<HTMLDivElement>(null),mapRef=useRef<MapLibreMap|null>(null),frameRef=useRef(0),resumeTimerRef=useRef(0),interactingRef=useRef(false),[sweeping,setSweeping]=useState(false)
 useEffect(()=>{if(!container.current)return
  ...
  map.once('load',async()=>{try{const geo=await fetch('/maps/south_cotabato_barangays_2023.geojson').then(response=>{if(!response.ok)throw new Error('Map unavailable');return response.json()}),byPsgc=new Map(rows.map(row=>[String(row.PSGC),row]))
   geo.features.forEach((feature:{properties:Record<string,string|number>})=>{const p=feature.properties,id=String(p.psgc??p.ORACLIS_PSGC??p.adm4_psgc??''),row=byPsgc.get(id);p.cases=row?.POSTERIOR_MEAN_CASES??0;p.risk=row?.OUTBREAK_PROBABILITY??.08})
