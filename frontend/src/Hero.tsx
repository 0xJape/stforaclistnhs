...
export default function Hero({rows,onEnter,leaving}:{rows:HeroRow[];onEnter:()=>void;leaving:boolean}){
 ...
 useEffect(()=>{rowsRef.current=rows;const geo=geoRef.current,source=mapRef.current?.getSource('hero-barangays') as maplibregl.GeoJSONSource|undefined;if(geo&&source){applyRows(geo,rows);source.setData(geo as GeoJSON.FeatureCollection)}},[rows])
